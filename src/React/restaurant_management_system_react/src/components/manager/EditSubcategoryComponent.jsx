...
function EditSubcategoryComponent() {
  ...
  const fetchSubcategoryById = async () => {
    try {
      const response = await axios.get(`http://localhost:8084/subcategories/getbyid/${id}`);
      setSubCategory({
        s_name: response.data.s_name,
        c_id: response.data.c_id.c_id // extract only the category ID
      });
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      c_id: { c_id: parseInt(subCategory.c_id) } // send object as expected by backend
    };

    try {
      await axios.put(`http://localhost:8084/subcategories/update/${id}`, payload);
  ...
  return (
    <div className="container mt-4">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="mb-3">
          <label htmlFor="c_id" className="form-label">Category ID</label>
          <input
            type="number"
            className="form-control"
            id="c_id"

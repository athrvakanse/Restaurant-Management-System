...
function ViewReservationsComponent() {
    ...
    const fetchReservations = () => {
        fetch(`http://localhost:8081/api/reservations/${loggedInUser.u_id}`)
            .then((res) => res.json())
            .then((data) => {
    ...
    const handleDeleteReservation = (rt_id) => {
        const confirmDelete = window.confirm(`Are you sure you want to delete reservation #${rt_id}?`);
        if (!confirmDelete) return;

        fetch(`http://localhost:8081/api/reservations/delete/${rt_id}`, {
            method: 'DELETE',
        })
            .then((response) => {
                if (response.ok) {

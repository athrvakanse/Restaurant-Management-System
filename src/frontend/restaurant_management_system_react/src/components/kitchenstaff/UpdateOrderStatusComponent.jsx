...
function UpdateOrderStatusComponent() {
  ...
  // Separate completed and other orders
  ...
  return (
    <div className="container mt-4">
      ...
      <div style={{ maxHeight: "500px", overflowY: "auto" }}>
        {activeOrders.length === 0 && <p>No active orders to update.</p>}
      ...
      <h3 className="mt-5">Completed Orders</h3>
      {completedOrders.length === 0 && <p>No completed orders yet.</p>}
      <div>
        {completedOrders.map(order => (
          <div key={order.o_id} className="card shadow p-3 mb-3 bg-light">
            <div className="card-body">
              <h5 className="card-title">Order #{order.o_id}</h5>
              <p>User: {order.u_id?.fname} {order.u_id?.lname}</p>
